...
<!--这里使用m-card m-card需要的icon和title必须传递,而这个组件内没有,在从外界接收进来,传递给m-card -->
  <m-card :icon="icon" :title="title" :dot="dot"> <!--使用m-card组件 在标签中添加的内容会被传递到m-card组件的插槽位置 -->
      ...
      <div class="pt-3"><!-- 轮播图滑动的事件.把轮播图的索引值给active-->
        <swiper :options="{autoHeight:true}" ref="list" @slide-change="()=>active=$refs.list.swiper.realIndex"> <!-- 使用swiper组件 生成对应tab栏的轮播图 到时候对应上 可以滑动切换tab -->
          <swiper-slide v-for="(item1,index1) in categories" :key="index1">  <!--根据父组件传递的数组循环 每一项是个对象 有一个name属性 和一个list的子数组 -->
            <slot name="items" :category='item1'></slot>  <!-- 把当前循环项 也就是当前的子数组的数据 通过作用域插槽+具名插槽传递给父组件 父组件通过 v-slot:items="category"来接受 接收到的数据就是每一个 item1子数组-->
          </swiper-slide>
        </swiper>
      </div>
